...
ADD CONSTRAINT bookings_info_length_check CHECK (info IS NULL OR length(info) <= 200);
...
BEGIN
  ...
  IF _info IS NOT NULL AND length(_info) > 200 THEN
    RAISE EXCEPTION 'Info must be 200 characters or fewer';
  END IF;
